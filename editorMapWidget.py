...
class EditorMap:
    ...
    def load_scenario(self, scenario):
        ...
        print(f"Geofence list length = {len(geofence_list)}")
        for i, drone_list in enumerate(geofence_list):
            self.polygon_count[i] = 0
            print(f"Drone list: {drone_list}")
            for j, shape in enumerate(drone_list):
                if shape["type"] == "polygon":
                    point_list = []
                    points = shape["waypoints"]
                    for point in points:
                        point_list.append([point["lat"], point["lon"]])
                    self.coords_list[i].append(point_list)
                    ...
                    polygon = self.map_widget.set_polygon(point_list,
                                                          ...
                                                          border_width=2)
                    print(f"Load scenario, appending shape {j}: {shape}")
    ...
    def click_first_marker(self):
        if len(self.markers[self.selected_drone_index]) >= 3:
            ...
            if self.polygon_count[self.selected_drone_index] < 1:
                ...
                fill_color = "black"
            self.polygon = self.map_widget.set_polygon(self.coords[self.selected_drone_index], fill_color=fill_color,
    ...
    def print_point(self, coords):
        ...
        if not self.check_marker_click(coords):
            ...
            marker = self.map_widget.set_marker(lat, lon, text=str(len(self.markers[self.selected_drone_index]) + 1),
                                                text_color="#ff0000", font=("Helvetica", 13, "bold"),
                                                icon=self.create_circle_icon())
            ...
            if len(self.coords[self.selected_drone_index]) > 1:
                if self.path[self.selected_drone_index]:
                    ...
                    self.map_widget.delete(self.path[self.selected_drone_index])
                self.path[self.selected_drone_index] = self.map_widget.set_path(self.coords[self.selected_drone_index],
                                                                                color="blue", width=2)
            else:
        ...
        if self.option_delete_all is True:
            if position == 0:
                for i in range(len(self.polygons[self.selected_drone_index])):
                    ...
                    del self.coords_list[self.selected_drone_index][0]
                    self.polygon_count[self.selected_drone_index] -= 1
            else:
                ...
                del self.coords_list[self.selected_drone_index][position]
                self.polygon_count[self.selected_drone_index] -= 1
        else:
            self.map_widget.delete(self.polygons[self.selected_drone_index][position])
            del self.polygons[self.selected_drone_index][position]
    ...
    def delete_marker_at_position(self, position):
        if position == 0:
            ...
            if len(self.markers[self.selected_drone_index]) > 0:
    ...
    def check_marker_click(self, coords):
        if self.first_point[self.selected_drone_index]:
            first_marker_coords = (self.first_point[self.selected_drone_index].position[0],
                                   self.first_point[self.selected_drone_index].position[1])
    ...
    def reset(self):
        ...
        for i, path in enumerate(self.path):
            if path:
                ...
                self.path[i] = None
        for drone in self.polygons:
            for polygon in drone:
    ...
    def clear_index(self, index):
        if self.init_complete:
            for marker in self.markers[index]:
                self.map_widget.delete(marker)
            self.map_widget.delete(self.path[index])
            self.path[index] = None

            for polygon in self.polygons[index]:
                self.map_widget.delete(polygon)

            self.markers[index] = []
            self.coords_list[index] = []
            self.coords[index] = []
            self.paths[index] = []
            self.path = None
            self.polygons[index] = []
